...
void handle_build(const std::string& path) {
...
#ifdef CIMPLE_USE_LLVM
	{
		...
		if (linker.link()) {
			...
			std::cerr << "[cimple] Linking failed. Object file available at: " << obj_file << "\n";
		}
	}
#else
	std::cout << "[cimple] LLVM backend not enabled. Install LLVM and rebuild with CIMPLE_USE_LLVM=ON\n";
#endif
}
